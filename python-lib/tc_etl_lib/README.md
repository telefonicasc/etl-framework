...
- Clase `cbManager`: En esta clase están funciones para la interacción con el Context Broker.
   - `__init()__`: constructor de objetos de la clase
        ...
        - :param opcional `need_auth_manager`: Se define que no es necesario autenticación para realizar operaciones con el Context Broker, por lo tanto no se enviarán las cabeceras X-Auth-Token en las peticiones. Por defecto es `True`, por lo tanto esperará que se le pase en las llamadas a las funciones, el authManager donde tendrá los tokens de autenticación. En el caso de marcarlo como `False`, no es obligatorio pasarle el authManager en las llamadas y por lo tanto, esperará que le especifiques en la llamada el servicio al que haces la operación con el parámetro `service`.
